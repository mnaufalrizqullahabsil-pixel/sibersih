// ============================================================
// SUPABASE SERVICE - FULL VERSION (ALL METHODS RESTORED)
// ============================================================
...
class SupabaseService {
  ...
  Future<void> changePassword(String oldPassword, String newPassword) async {
    try {
      // Supabase menyarankan update password saat user sedang dalam session aktif
  ...
  Future<bool> submitLaporan({required List<Uint8List> fotoBytes, required double berat, required String lokasi, required String catatan}) async {
    final authUser = _supabase.auth.currentUser;
    ...
    try {
      ...
      await _supabase.from('laporans').insert({
        'user_id': authUser.id, 'nama_pelapor': _currentUser?.nama, 'nim': _currentUser?.nim,
        'foto_urls': urls, 'berat': berat, 'lokasi': lokasi, 'catatan': catatan, 'status': 'pending', 'tanggal': DateTime.now().toIso8601String(),
      });
      return true;
  ...
  Future<List<LaporanModel>> getRiwayatLaporan() async {
    ...
    if (authUser == null) return [];
    try {
      final List<dynamic> data = await _supabase.from('laporans').select().eq('user_id', authUser.id).order('tanggal');
  ...
  Future<List<Map<String, dynamic>>> getLeaderboard({int limit = 10}) async {
    try {
      final List<dynamic> data = await _supabase.from('users').select().order('total_poin', ascending: false).limit(limit);
  ...
  static const _mockUser = UserModel(uid: '0', nama: 'Guest', nim: '0', jurusan: '-', email: '', totalPoin: 0, poinMasuk: 0, poinKeluar: 0, rank: 0, jumlahLaporan: 0, level: 'Pemula');
}
